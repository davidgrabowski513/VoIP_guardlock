...
- (instancetype)initWithPerson:(ABRecordRef)aperson {
	...
	LOGI(@"Contact %@ initialized with %d phones, %d sip, %d emails", self.name ?: @"",
...
//		_lastName = (NSString *)CFBridgingRelease(ABRecordCopyValue(person, kABP?ersonLastNameProperty));
        _name = (NSString *)CFBridgingRelease(ABRecordCopyValue(person, kABPersonFirstNameProperty));
	}
...
-(void)setName:(NSString *)name {
    if ([self replaceInProperty:kABPersonFirstNameProperty value:(__bridge CFTypeRef)(name)]){
        _name = name;
    }
}
