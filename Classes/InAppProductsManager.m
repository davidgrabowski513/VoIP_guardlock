...
//		_available = false; // must be disappear??? - test subscription for every sign up
		[[SKPaymentQueue defaultQueue] addPayment:payment];
		[[SKPaymentQueue defaultQueue] addTransactionObserver:self];
        
        //added new forces subscriptions
        [_productIdCurrent addObject:productID];
...
//												 inSection:@"in_app_purchase"];
        NSString *productID = monthly ? @"il.co.guardlock.guardphone.ios.guardlock.monthly" : @"il.co.guardlock.guardphone.ios.product.yearly";
...
- (BOOL)isAvailableAccount {
//    NSData *receipt; // Sent to the server by the device
    NSString *receiptString = [self getReceipt];
                           ...
                           completionHandler:^(NSURLResponse *response, NSData *data, NSError *connectionError) {
                               if (connectionError) {
                                   ...
                                   /* ... Send a response back to the device ... */
                                       ...
                                       NSString *expired_info = (NSString *)[jsonResponse valueForKey:@" latest_expired_receipt_info"];
                                       if (expired_info){
                                           [self postNotificationforStatus:kIAPPurchaseExpired withDict:nil];//expired
                                       } else {
...
//								  withString:@""] componentsSeparatedByString:@","];
    ...
    NSArray *list = [[NSArray alloc] initWithObjects:@"il.co.guardlock.guardphone.ios.guardlock.monthly", @"il.co.guardlock.guardphone.ios.product.yearly", nil];
	_productsIDPurchased = [[NSMutableArray alloc] initWithCapacity:0];
    
    _productIdCurrent = [[NSMutableArray alloc] initWithCapacity:0];
...
- (void)requestDidFinish:(SKRequest *)request {
	if ([request isKindOfClass:[SKReceiptRefreshRequest class]]) {
		...
		if ([[NSFileManager defaultManager] fileExistsAtPath:[receiptUrl path]]) {
            ...
            //must remove user account here
            [self postNotificationforStatus:kIAPPurchaseExpired withDict:nil];
		}
	}
}
...
//		NSURL *URL = [NSURL URLWithString:[LinphoneManager.instance lpConfigStringForKey:@"receipt_validation_url"
//																			   inSection:@"in_app_purchase"]];
//        NSURL *URL = [NSURL URLWithString:@"https://sandbox.itunes.apple.com/verifyReceipt"];   //this is the test enviroment url
        NSURL *URL = [NSURL URLWithString:@"https://buy.itunes.apple.com/verifyReceipt"];
		XMLRPCRequest *request = [[XMLRPCRequest alloc] initWithURL:URL];
		// buying for the first time: need to create the account
		// if ([transaction.transactionIdentifier
		// isEqualToString:transaction.originalTransaction.transactionIdentifier]) {
		if (self.accountCreationData.count == 0) { // origin = 3
			[request setMethod:@"create_account_from_in_app_purchase"
				withParameters:[NSArray arrayWithObjects:@"", [_accountCreationData objectForKey:@"phoneNumber"],
														 receiptBase64, IAP_SHARED_SECRET_KEY, @"apple",
			...
			if ([[self getPhoneNumber] length] > 0) {
				[request setMethod:@"get_expiration_date"
					withParameters:[NSArray
									   arrayWithObjects:[self getPhoneNumber], receiptBase64, IAP_SHARED_SECRET_KEY, @"apple", nil]];
			} else {
...
- (void)paymentQueue:(SKPaymentQueue *)queue updatedTransactions:(NSArray *)transactions { // response for pay request
	for (SKPaymentTransaction *transaction in transactions) {
		switch (transaction.transactionState) {
			...
			case SKPaymentTransactionStateFailed: {
                [self postNotificationforStatus:kIAPPurchaseExpired withDict:nil]; //remove user account on server
