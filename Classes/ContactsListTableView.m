...
-(void)removeData{
    NSIndexPath* indexPath = [Constants getIndexPath];
    ABAddressBookUnregisterExternalChangeCallback(addressBook, sync_address_book, (__bridge void *)(self));
...
- (NSArray<UITableViewRowAction *> *)tableView:(UITableView *)tableView editActionsForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    UITableViewRowAction *editAction = [UITableViewRowAction rowActionWithStyle:UITableViewRowActionStyleNormal title:@"Edit" handler:^(UITableViewRowAction *action, NSIndexPath *indexPath){
        UILabel *nameField = [[tableView cellForRowAtIndexPath:indexPath] viewWithTag:9];
        UILabel *numberField = [[tableView cellForRowAtIndexPath:indexPath] viewWithTag:10];
        
        [ContactSelection setAddAddress:numberField.text];
        ContactDetailsView *view = VIEW(ContactDetailsView);
       
        [Constants setUpdateValue:1];
        [Constants saveIndexPath:indexPath];
        
        [PhoneMainView.instance changeCurrentView:view.compositeViewDescription];

        [view newContact:[ContactSelection getAddAddress] withName:nameField.text];
