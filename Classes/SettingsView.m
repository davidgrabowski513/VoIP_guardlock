...
- (NSSet *)findHiddenKeys {
    ...
    [hiddenKeys addObject:@"audio_menu"];               //hide options audio_menu
    
    //hide "change password" & "manage subscriptions" 20160805
    [hiddenKeys addObject:@"mng_subs_button"];
    [hiddenKeys addObject:@"ch_password_button"];
...
- (void)settingsViewControllerWillAppear:(IASKAppSettingsViewController *)sender {
    _isRootView = (sender.file == nil || [sender.file isEqualToString:@"Root"]);
...
- (void)settingsViewController:(IASKAppSettingsViewController *)sender
    ...
    if ([key isEqual:@"assistant_button"]) {
		...
		[alert show];
    } else if ([key isEqualToString:@"ch_password_button"]){
        AssistantView *view = VIEW(AssistantView);
        [view addViewToHistory:self];
        [PhoneMainView.instance changeCurrentView:view.compositeViewDescription];
        [view gotoChagePassword:self];
    } else if ([key isEqualToString:@"mng_subs_button"]){
//        [[UIApplication sharedApplication] openURL:[NSURL URLWithString:UIApplicationOpenSettingsURLString]];//this opens app's settings
        [[UIApplication sharedApplication] openURL:[NSURL URLWithString:@"prefs://"]]; //this opens the first setting view - first register url types in info.plist
    }
}
