...
- (UIView *)tableView:(UITableView*)tableView viewForHeaderInSection:(NSInteger)section {
    if (section == 1){ // show user account
        UILabel *label = [[UILabel alloc] init];
        label.text=[Constants getAddress];
...
- (CGFloat)tableView:(UITableView*)tableView heightForHeaderInSection:(NSInteger)section {
	...
	if ((title = [self tableView:tableView titleForHeaderInSection:section])) {
        ...
        if ([title isEqualToString:@"Username"]){
            return -1;
        }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if ([specifier.type isEqualToString:kIASKPSToggleSwitchSpecifier]) {
        ...
        if ([specifier.key isEqualToString:@"txtUsername"]){
            cell.textLabel.text = [Constants getAddress];
        ...
        if ([specifier.key isEqualToString:@"assistant_button"]){
