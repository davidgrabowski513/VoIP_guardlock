...
NSString *const kLinphoneHidebuttonSetting = @"LinphoneHidebuttonSetting";
...
//												   {"silk", 16000, "silk_16k_preference"},          //set off Silk 24kHZ
...
//												   {"opus", 48000, "opus_preference"},              //set off Opus menu
												   {NULL, 0, Nil}};
...
- (void)onRegister:(LinphoneCore *)lc
    ...
    if ([[NSString stringWithUTF8String:message] isEqualToString:@"Bad Request"]){ // this prevents critical forcely
        AssistantView *view = VIEW(AssistantView);
        [Constants setReSignWithValue:YES];
        [view reset];
        [view onLoginClick:self];
        return;
    }
...
void networkReachabilityCallBack(SCNetworkReachabilityRef target, SCNetworkReachabilityFlags flags, void *nilCtx) {
	...
	if (theLinphoneCore != nil) {
		...
		if ((flags == 0) || (flags & networkDownFlags)) {
			...
			BOOL isWifiOnly = [lm lpConfigBoolForKey:@"wifi_only_preference" withDefault:NO];
            isWifiOnly = FALSE; // this removes the critical BUG!!!
			if (!ctx || ctx->testWWan)
            ...
            if (isProg){ // this remove the critical changing network state
                //                    linphone_core_refresh_registers(LC);
                //                    [lm enterBackgroundMode];
                isProg = NO;
//                [lm enterBackgroundMode];
            }
  		}
...
//	linphone_core_iterate(theLinphoneCore);
	// start scheduler
	mIterateTimer =
		[NSTimer scheduledTimerWithTimeInterval:1 target:self selector:@selector(iterate) userInfo:nil repeats:YES];
}
