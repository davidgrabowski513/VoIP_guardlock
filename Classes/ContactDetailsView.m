...
- (void)addCurrentContactContactField:(NSString *)address withName: (NSString *)name{
    LinphoneAddress *linphoneAddress = linphone_core_interpret_url(LC, address.UTF8String);
    ...
    if (([username rangeOfString:@"@"].length > 0) &&
        ...
        [_tableController addSipField:address];
        [_tableController addNameField:name];
...
- (void)newContact:(NSString *)address withName: (NSString *)name{
    [self selectContact:ABPersonCreate() andReload:NO];
    [self addCurrentContactContactField:address withName:name];
    
    [Constants saveContactUsername:name];
}
...
- (IBAction)onEditClick:(id)event {
	if (_tableController.isEditing) {
        ...
        if ([Constants getUpdateValue] == 1){
            [Constants setUpdateValue:2];
            
            _mContact = _contact;
            
            ContactsListView *view = VIEW(ContactsListView);
            [view.tableController removeData];
            
            _contact = _mContact;
            
            NSString * test = _tableController.contact.name;
            if ([test isEqualToString:[Constants getContactUsername]]){
                [Constants setIsRemove:NO];
            } else {
                [Constants setIsRemove:YES];
            }
        }
