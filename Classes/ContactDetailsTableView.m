...
//	} else
    if (path.section == ContactSections_Sip) {
...
//	} else
    ...
    } else if (section == ContactSections_Name){
        added = [_contact addName:value];
        _contact.name = value;
//        [_contact setName:value];
    }
...
- (void)addNameField:(NSString *)name {
    ContactSections i = 0;
    while (i != ContactSections_MAX && i != ContactSections_Name)
        ++i;
    [self addEntry:[self tableView] section:i animated:FALSE value:name];
}

- (BOOL)isValid {
//	return _contact.firstName.length + _contact.lastName.length > 0;
    return _contact.name.length > 0;
}
...
//	if (section == ContactSections_FirstName || section == ContactSections_LastName)
    if (section == ContactSections_Name){
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
	...
	[cell hideDeleteButton:YES]; // remove the options to remove phones and sip address
	[cell.editTextfield setKeyboardType:UIKeyboardTypeDefault];
	NSString *value = @"";
	if (indexPath.section == ContactSections_Name) {
        value = _contact.name;
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section {
    ...
    canAddEntry = NO;       //remove the options to add more or remove phones and sip address
	NSString *addEntryName = nil;
	if (section == ContactSections_Name && self.tableView.isEditing) {
...
//		(!self.tableView.isEditing && (section == ContactSections_FirstName || section == ContactSections_LastName)))
    if (section == 0 || (!self.tableView.isEditing && (section == ContactSections_Name)))
    {
		return 1e-5;
	}
